...
\begin{enumerate}[label=\alph*.]
    ...
    \end{itemize}
    \item \textbf{Aristas desde la Fuente:}  
    ...
    \item \textbf{Aristas Intermedios:}  
    ...
    \item \textbf{Aristas al Sumidero:}  
...
\begin{itemize}
    ...
    \item \textbf{Aristas:} Cada arco se representa como una estructura o tupla con información sobre el nodo origen, nodo destino, capacidad máxima y flujo actual.
\end{itemize}
...
\begin{tikzpicture}[node distance=1.5cm and 1.5cm, auto,
    ...
    % Aristas desde S hacia los nodos "in_i" (capacidad k=1)
    \draw[->] (S) -- node[above] {1} (in0);
    \draw[->] (S) -- node[above] {1} (in1);
    \draw[->] (S) -- node[above] {1} (in2);
    
    % Aristas desde los nodos "out_i" hacia T (capacidad b=1)
    \draw[->] (out0) -- node[above] {1} (T);
    \draw[->] (out1) -- node[above] {1} (T);
    \draw[->] (out2) -- node[above] {1} (T);
    
    % Aristas entre nodos "in_i" y "out_j" (i ≠ j, capacidad 1)
...
\begin{tikzpicture}[node distance=1.5cm and 1.5cm, auto,
    ...
    % Aristas desde S hacia los nodos "in_i" (dibujo en gris)
    \draw[->, dashed] (S) -- (in0);
    \draw[->, dashed] (S) -- (in1);
    \draw[->, dashed] (S) -- (in2);
    
    % Aristas desde los nodos "out_i" hacia T (dibujo en gris)
    \draw[->, dashed] (out0) -- (T);
    \draw[->, dashed] (out1) -- (T);
    \draw[->, dashed] (out2) -- (T);
    
    % Aristas candidatos (sin seleccionar, en gris)
    ...
    % Aristas seleccionados (solución óptima, en rojo y gruesos)
...
\begin{itemize}
    ...
    \item  Obtener longitud de matriz: $\mathcal{O}(1)$

    \item  Comparación: $\mathcal{O}(1)$

    \item  Inicializar arreglo de $n$ listas vacías: $\mathcal{O}(n)$

    \item   Doble bucle sobre nodos ($n \times n$) y acceso al flujo:
    \begin{itemize}
...
\subsection{Análisis de complejidad y resultados}

\begin{table}[ht]
    \centering
    \begin{tabular}{cc}
        \hline
        \textbf{Matriz \(n \times n\)} & \textbf{Tiempo (ms)} \\ \hline
        4N & 2.3677 \\
        9N & 2.4239 \\
        16N & 2.7826 \\
        25N & 3.5207 \\
        36N & 4.8964 \\ \hline
    \end{tabular}
    \caption{Tiempos de ejecución (en milisegundos) para diferentes tamaños de matriz.}
    \label{tab:tiempos}
\end{table}
...
\begin{axis}[
    width=12cm,
    height=8cm,
    xlabel={Tamaño de la matriz (n x n)},
    ylabel={Tiempo (ms)},
    xtick=data,
    xticklabel style={rotate=45},
    symbolic x coords={4N, 9N, 16N, 25N, 36N},
    ymin=0,
    grid=major,
    nodes near coords,
    every node near coord/.append style={font=\footnotesize},
    title={Tiempo de ejecución según el tamaño de la matriz},
]
\addplot[
    color=blue,
    mark=*,
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            xlabel={$x$},
            ylabel={$f(x)=x^5$},
            domain=0:6,        % Dominio para x
            samples=1000,        % Más muestras para suavizar la curva
            grid=both,
            axis lines=middle,
            xmin=0, xmax=6,
            ymin=0, ymax=7776,   % Ajusta según el rango deseado
            axis lines=middle,  % Dibuja los ejes en posición central
            enlargelimits=true
        ]
            \addplot[blue, thick] {x^5};
        \end{axis}
    \end{tikzpicture}
    \caption{Gráfico de la función \(x^5\).}
\end{figure}
\subsection{Conclusión de análisis de complejidad}
\text Los resultados muestran que tiende a cumplir con la curva esperada, a pesar de no ser tan exponencial su escalada en comparación con los datos ingresados.
